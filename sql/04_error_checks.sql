...
-- UTF-8 multibyte characters misread as Latin-1 in authored name fields.
-- Covers all pindata entity types with free-text names.
-- Never checks aliases (which intentionally store garbled source variants).
INSERT INTO _violations
...
UNION ALL
SELECT 'mojibake', 'location_name', slug FROM locations WHERE is_mojibake(name)
UNION ALL
SELECT 'mojibake', 'manufacturer_name', slug FROM manufacturers WHERE is_mojibake(name)
UNION ALL
SELECT 'mojibake', 'model_name', slug FROM models WHERE is_mojibake(name)
UNION ALL
SELECT 'mojibake', 'person_name', slug FROM people WHERE is_mojibake(name)
UNION ALL
SELECT 'mojibake', 'series_name', slug FROM series WHERE is_mojibake(name)
UNION ALL
SELECT 'mojibake', 'theme_name', slug FROM themes WHERE is_mojibake(name)
UNION ALL
SELECT 'mojibake', 'title_name', slug FROM titles WHERE is_mojibake(name);
...
FROM ref_location_country_aliases
GROUP BY alias
HAVING count(*) > 1;
