...
Everything HTML-specific lives here: reading the charset a page declares about
itself (the second opinion ``charset`` needs for the headerless Shift-JIS case
old Japanese pages hit), converting the whole document body to block-level
markdown with markdownify, carrying an allowlist of ``<head>`` metadata into the
text, and extracting a conservative date. The transport and the fetcher reach
this only through the ``ContentHandler`` interface — they never mention HTML.
...
class HtmlHandler(ContentHandler):
    ...
    def decode(self, raw: bytes, header_charset: str | None) -> str | None:
        # The <meta> declaration is HTML's own second opinion when the server
        # sent no charset — the headerless Shift-JIS case.
        return charset.decode_body(raw, header_charset, _sniff_meta_charset(raw))

    @override
    def rereads_faithfully(self, raw: bytes) -> bool:
        # A page carries its charset only if it declared one; a cp1252 page that
        # left it to the header re-reads by guesswork.
        return charset.resolves_without_detection(raw, _sniff_meta_charset(raw))
