...
- ✅ DONE: **Comparison layer in Flippatch**. `scripts/analysis/external_data_sources/` in flippatch: `bridge.sql` (the attach, the `external_data_sources_context` watermark view, one `external_data_source_findings` worklist across sources modelled on but parallel to the Flipcommons audit, dismissals with date + note, and a boundary check that fails any read past the pinexplore marts) plus `ipdb.sql` and `opdb.sql`. Findings are views, never build-blocking; `error` is reserved for "our data is currently wrong". Documented in flippatch's `docs/AGENTS.src.md` under "Comparing external data sources against the catalog".
...
- ✅ DONE: **Move the OPDB manufacturer exceptions to Flippatch**. The 13 researched rows live in `opdb.sql`'s `_eds_opdb_manufacturer_exceptions` (the rotted `mecatronics-…` slug corrected to `mecatronics`), with the slug-resolution check its old comment asked for (`exception_slug_unresolved` in `opdb_checks`). Two rows no longer match any model pair — OPDB re-filed Jocmatic under its own id 172, and the Segasa/Sonic filing no longer surfaces — reported as `manufacturer_exceptions_stale` in `opdb_summary`, never gated; prune them when convenient.
- MOSTLY DONE: **Widen `opdb_mart_view_undocumented` past `opdb`**. Now `mart_relation_undocumented`, covering `opdb`, `ipdb` and `ingest` — every relation there carries a `COMMENT`. Still excluded: `glossary` (`glossary.compared` is uncommented) and `web_cache`; comment those and add the schemas to the check's list.
